        ...
        Papa.parse(SHEET_URL, {
            ...
            complete: function (results) {
                ...
                const mapped = data.map((row, index) => {
                    ...
                    return {
                        id: String(index),
                        rawInfo: rowVals.join(' ').toLowerCase(),
...
function setupAppListeners() {
    searchInput.addEventListener('input', (e) => {
        ...
        if (!term) {
            ...
            // Remove common question words/articles to extract the actual keywords
            const keywords = term.replace(/\b(cuantos|cuántos|tienen|quienes|quiénes|son|los|las|el|la|un|una|de|en|por)\b/g, '').trim().split(/\s+/).filter(k => k);
            
            currentStudents = studentsData.filter(user => {
                if (keywords.length === 0) return true;
                // Require all extracted keywords to match somewhere in the row's raw values
            ...
            <div class="student-card bg-white rounded-2xl shadow-sm border ${hasWarning ? 'border-red-200' : 'border-gray-200'} overflow-hidden flex flex-col cursor-pointer" onclick="openStudentModal('${student.id}')">
                <div class="p-5 flex-grow">
                    ...
                    <div class="flex gap-2 text-xs">
                        <span class="bg-gray-100 px-2 py-1 rounded-md text-gray-600 font-medium">${student.edad} años</span>
        ...
        <div class="${hasWarning ? 'bg-red-50 border-red-100' : 'bg-indigo-50 border-indigo-100'} p-6 border-b rounded-t-2xl">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-gray-600 font-medium">${student.tipoDoc} ${student.documento} • ${student.edad} años</p>
                </div>
            </div>
        </div>
