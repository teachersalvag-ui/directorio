        ...
        Papa.parse(SHEET_URL, {
            ...
            complete: function (results) {
                ...
                const mapped = data.map((row, index) => {
                    ...
                    let fotoUrl = row["foto"] || row["Foto"] || row["FOTO"] || "";
                    if (fotoUrl.includes("drive.google.com")) {
                        const matchId = fotoUrl.match(/[-\w]{25,}/);
                        if (matchId && matchId[0]) {
                            fotoUrl = `https://drive.google.com/uc?export=view&id=${matchId[0]}`;
                        }
                    }
                    return {
                        foto: fotoUrl,
                        id: String(index),
                        rawInfo: (rowVals.join(' ') + ' ' + (row["¿Es usted colombiano?"] === 'No' ? 'extranjero' : 'colombiano') + ' ' + (row["Edad"] ? row["Edad"] + ' años' : '')).toLowerCase(),
                        ...
                        edad: row["Años"] || rowVals[8] || "N/A",
                        edadNumerica: row["Edad"] || rowVals[7] || "N/A",
                        genero: row["Genero"] || row["Género"] || rowVals[17] || "N/A",
            ...
            <div class="student-card bg-white rounded-2xl shadow-sm border ${hasWarning ? 'border-red-200' : 'border-gray-200'} overflow-hidden flex flex-col cursor-pointer" onclick="openStudentModal('${student.id}')">
                <div class="p-5 flex-grow">
                    <div class="flex justify-between items-start mb-3">
                        <div class="w-12 h-12 rounded-full bg-indigo-100 text-indigo-700 font-bold text-xl flex items-center justify-center overflow-hidden">
                            ${student.foto ? `<img src="${student.foto}" alt="${student.nombre}" class="w-full h-full object-cover">` : student.nombre.charAt(0)}
                        </div>
    ...
    modalContent.innerHTML = `
        ...
        <div class="${hasWarning ? 'bg-red-50 border-red-100' : 'bg-indigo-50 border-indigo-100'} p-6 border-b rounded-t-2xl">
            <div class="flex items-center gap-4">
                <div class="w-16 h-16 rounded-full bg-white text-gray-700 shadow-sm font-bold text-2xl flex items-center justify-center overflow-hidden border-2 border-white">
                    ${student.foto ? `<img src="${student.foto}" alt="${student.nombre}" class="w-full h-full object-cover">` : student.nombre.charAt(0)}
                </div>
                <div>
